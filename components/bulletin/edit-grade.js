...
const BulletinRowEdit = ({ item, showModal, onDataChange }) => {
  const { register, handleSubmit, watch, errors } = useForm();
  const onSubmit = (data) => {
    const ediData = {...item, modified: true};
  ...
  const Input = ({ label, data, register, errors }) => (
    <>
      <InputMask
        name={label}
        ...
        defaultValue={data}
      />
      {errors[label] && <span>Last name is required</span>}
    </>
  );
...
export default BulletinRowEdit;
